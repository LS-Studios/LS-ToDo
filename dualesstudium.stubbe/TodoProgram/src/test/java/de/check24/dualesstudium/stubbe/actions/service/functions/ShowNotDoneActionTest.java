...
class ShowNotDoneActionTest {

    private TodoView view;
    private TodoDataLayerInterface todoDataLayerInterface;
    MockedStatic<ActionHelper> utility;
    private ShowNotDoneAction showNotDoneAction;

    @BeforeEach
    void setup() {
        this.view = mock(TodoView.class);
        this.todoDataLayerInterface = mock(TodoDataLayerInterface.class);
        this.showNotDoneAction = new ShowNotDoneAction(todoDataLayerInterface, view, "show not done");
    ...
    void runTest() {
        ...
        utility.verify(() -> ActionHelper.showTodo(any(Todo.class), eq(view)), times(2));
    }
